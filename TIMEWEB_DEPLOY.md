...
>
> `PS_YM_ID` — при деплое подставляется в `js/config.js`; в репозитории задан счётчик **`110523171`**.
...
### Плашки Timeweb: «MX неверно» / «SPF неверно»

Это **DNS домена** `pacificstar.ru`, не код сайта. Сейчас у домена **две SPF-записи сразу** — так нельзя, из‑за этого Timeweb и ругается.

**Проверка (уже сейчас):**
- MX: `mx.yandex.net` — нормально, если ящики `sales@` / `noreply@` на **Яндекс.Почте**
- SPF: **две** TXT — `redirect=_spf.yandex.net` **и** `include:_spf.timeweb.ru` ← **ошибка**

**Исправление (5 минут):**

1. Timeweb → **Домены** → `pacificstar.ru` → **DNS / Настройки DNS**
2. **Удалить** обе TXT-записи SPF (с `yandex` и с `timeweb` в тексте)
3. **Добавить одну** TXT для `@`:

```
v=spf1 include:_spf.timeweb.ru include:_spf.yandex.net ~all
```

4. MX **не трогать**, если почта на Яндексе — должна остаться:

| Тип | Хост | Значение | Приоритет |
|-----|------|----------|-----------|
| MX | `@` | `mx.yandex.net` | 10 |

> Плашка «MX неверно» может остаться, если Timeweb ждёт *свои* MX (`mx1.timeweb.ru`). Для Яндекс-почты это **ложное предупреждение** — главное, чтобы MX был `mx.yandex.net`.
...
Ранее использовался Node.js на App Platform — **переменные окружения в панели Timeweb туда так и не дошли**. Формы переведены на PHP; **настраивать ENV в App Platform больше не нужно**.

**Рекомендуется:** [дашборд #220769](https://timeweb.cloud/my/apps/220769/dashboard) → **Остановить** или удалить приложение. Логи `SMTP_USER=(empty)` и `ENV missing` — от этого старого Node-процесса, на сайт не влияют.

Проверка почты: `https://pacificstar.ru/api/health.php` → `"smtp":true`.

### App Platform (если позже понадобится отдельный Node API)
