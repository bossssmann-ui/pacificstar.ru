...
#### ❗ Проблемы с оплатой или верификацией телефона

При добавлении способа оплаты на странице **[Billing](https://console.anthropic.com/settings/billing)** Anthropic может запросить номер телефона для верификации. Если после ввода номера появляется ошибка **«There was a problem, please try again later»** (или «Произошла ошибка, попробуйте позже») — вот что можно сделать:

| Проблема | Решение |
|----------|---------|
| Не принимает номер телефона из РФ | Anthropic не поддерживает российские номера для SMS-верификации. Попробуй ввести номер в международном формате `+7 9XX XXX XX XX`. Если не помогает — потребуется номер другой страны (например, виртуальный номер через онлайн-сервис) |
| Не принимает российскую банковскую карту | Карты банков РФ (Visa/MasterCard/Мир) заблокированы из-за санкций. Используй **виртуальную карту** зарубежного платёжного сервиса или карту иностранного банка |
| Общая ошибка при оплате | Попробуй: 1) другой браузер (Chrome/Firefox), 2) очистить кеш и куки, 3) отключить VPN (или наоборот — включить VPN через другую страну), 4) подождать и попробовать позже — бывают временные сбои на стороне Anthropic |
| Не приходит SMS-код подтверждения | Проверь что номер введён верно. Нажми «Resend code». Подожди 2–3 минуты. Если код приходит на email — проверь папку «Спам» |

> 💡 **Альтернатива:** Если оплата через Anthropic Console полностью недоступна, можно использовать Claude Code через
...
> эти провайдеры принимают другие способы оплаты.
