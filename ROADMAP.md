...
### Вариант B — Отдельный VPS / serverless

1. Деплой `server.js` на VPS (PM2) или Railway / Render.
2. CORS + прокси: формы шлют на `https://api.pacificstar.ru/api/contact`.
3. Обновить `fetch('/api/contact')` в `main.js` на абсолютный URL (через env/config).

### Вариант C — Внешний сервис форм (быстрый старт)
