...
### Предложение контракта (НЕ согласовано — на утверждение владельцу)

Для реального стека (статический HTML/CSS/JS + PHP-бэкенд форм) предлагается 3 содержательные проверки на `pull_request` + отдельная проверка публикации:

1. **GitGuardian Security Checks** (уже есть) — сканирование секретов в диффе.
2. **HTML validation** (`html-validate` по изменённым `*.html`) — сайт статический, битую разметку merge публикует сразу в prod. Ввести сначала как report-only; сделать обязательной после чистки контента (сейчас падает на `main`).
3. **PHP lint** (`php -l` по `api/*.php`) — production-доставка заявок на PHP; синтаксическая ошибка ломает `contact/callback/register`. Дёшево, ставится сразу как обязательная (CI ставит `php-cli`, как деплой).

Дополнительно (не PR-гейт): **проверка публикации после merge** — job `🚀 Загрузить сайт на Timeweb` уже делает smoke HTTP 200; закрепить как обязательный шаг релиза. Опционально `cspell` — как отчётная проверка качества до чистки контента. Контракт не внедрять без согласования и не выдавать за уже действующий.
