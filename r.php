<?php
// r.php — финальный установщик Pacific Star (без перенаправления)
// Запишет index.html и .htaccess, покажет результат
$dir = rtrim(__DIR__, '/');
$results = [];
...
$ok_htaccess = file_put_contents($dir . '/.htaccess', $htaccess) !== false;
$ok_index    = file_put_contents($dir . '/index.html', $index_html) !== false;
$all_ok      = $ok_htaccess && $ok_index;
?>
<!DOCTYPE html>
<html lang="ru">
<head>
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  body{font-family:Arial,sans-serif;background:<?= $all_ok ? "#0d3b1e" : "#3b0d0d" ?>;
       color:#fff;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px}
  .box{background:rgba(255,255,255,.08);border-radius:16px;padding:40px 32px;max-width:520px;width:100%;
       border:2px solid <?= $all_ok ? "#4caf50" : "#f44336" ?>}
  .icon{font-size:64px;text-align:center;margin-bottom:16px}
  h1{font-size:24px;text-align:center;margin-bottom:24px;
     color:<?= $all_ok ? "#69f0ae" : "#ff5252" ?>}
  .row{display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid rgba(255,255,255,.1)}
  .row:last-of-type{border-bottom:none}
  .dot{font-size:22px}
  .lbl{font-size:15px}
  .btn{display:block;margin:28px auto 0;padding:16px 32px;background:#1565c0;color:#fff;
       border:none;border-radius:10px;font-size:18px;font-weight:bold;cursor:pointer;
       text-decoration:none;text-align:center;max-width:320px;letter-spacing:.5px}
  .btn:hover{background:#1976d2}
  .hint{margin-top:14px;text-align:center;font-size:13px;color:#aaa;line-height:1.5}
  .err-msg{background:rgba(244,67,54,.15);border:1px solid #f44336;border-radius:8px;
           padding:12px;margin-top:16px;font-size:13px;line-height:1.6}
</style>
</head>
<body>
<div class="box">
  <div class="icon"><?= $all_ok ? "🎉" : "❌" ?></div>
  <h1><?= $all_ok ? "Сайт установлен!" : "Ошибка установки" ?></h1>

  <div class="row">
    <span class="dot"><?= $ok_htaccess ? "✅" : "❌" ?></span>
    <span class="lbl">.htaccess — <?= $ok_htaccess ? "перезаписан (правила Tilda удалены)" : "ошибка записи" ?></span>
  </div>
  <div class="row">
    <span class="dot"><?= $ok_index ? "✅" : "❌" ?></span>
    <span class="lbl">index.html — <?= $ok_index ? "заменён (старый Tilda → наш сайт)" : "ошибка записи" ?></span>
  </div>

  <?php if($all_ok): ?>
  <a class="btn" href="/" onclick="return confirm('Нажмите ОК и затем на открывшейся странице нажмите Ctrl+F5')">
    🌐 Открыть pacificstar.ru
  </a>
  <p class="hint">После открытия сайта нажмите <strong>Ctrl+F5</strong> (или Cmd+Shift+R на Mac)<br>
  чтобы браузер загрузил свежую версию, а не старую из кэша</p>
  <?php else: ?>
  <div class="err-msg">
    Напишите в поддержку Timeweb:<br>
    <em>«Нужны права на запись в /public_html. Файлы .htaccess и index.html не перезаписываются»</em>
  </div>
  <?php endif; ?>
</div>
</body>
